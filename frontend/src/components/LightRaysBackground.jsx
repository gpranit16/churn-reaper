...
export default function LightRaysBackground({
  ...
  useEffect(() => {
    ...
    const loop = (t) => {
      ...
      if (!rendererInstance || !uniformsInstance) return;

      uniformsInstance.iTime.value = t * 0.001;

      if (followMouse && mouseInfluence > 0) {
        const smoothing = 0.92;
        smoothMouseRef.current.x = smoothMouseRef.current.x * smoothing + mouseRef.current.x * (1 - smoothing);
        smoothMouseRef.current.y = smoothMouseRef.current.y * smoothing + mouseRef.current.y * (1 - smoothing);
        uniformsInstance.mousePos.value = [smoothMouseRef.current.x, smoothMouseRef.current.y];
      }
